...
impl ContainerDefinition {
  pub fn to_rusoto(&self) -> rusoto_ecs::ContainerDefinition {
    rusoto_ecs::ContainerDefinition {
      ...
      environment: self.environment.as_ref().map(|e| e.iter().map(|e0| e0.to_rusoto()).collect()),
      essential: self.essential,
      extra_hosts: self.extra_hosts.as_ref().map(|e| e.iter().map(|e0| e0.to_rusoto()).collect()),
      ...
      mount_points: self.mount_points.as_ref().map(|e| e.iter().map(|e0| e0.to_rusoto()).collect()),
      name: self.name.to_owned(),
      port_mappings: self.port_mappings.as_ref().map(|e| e.iter().map(|e0| e0.to_rusoto()).collect()),
      privileged: self.privileged,
      readonly_root_filesystem: self.readonly_root_filesystem,
      ulimits: self.ulimits.as_ref().map(|e| e.iter().map(|e0| e0.to_rusoto()).collect()),
      user: self.user.to_owned(),
      volumes_from: self.volumes_from.as_ref().map(|e| e.iter().map(|e0| e0.to_rusoto()).collect()),
