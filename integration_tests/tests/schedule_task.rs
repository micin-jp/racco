...
fn schedule_task_put() {
  ...
  let conf = config::command::Config::from_file("fixtures/configs/schedule_task.yml", None).unwrap();
  let cmd = schedule_task::put::Command::new(&conf, "racco-test-schedule-job");
